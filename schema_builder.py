...
def wire_relationships():
    ...
    for t_key, t_def in TEMPLATES.items():
        ...
        for child_key, child_def in TEMPLATES.items():
            if child_key == t_key: continue
            
            refs = child_def.get("references", {})
            for child_field_ref, ref_conf in refs.items():
